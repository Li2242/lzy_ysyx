...
AM_DEVREG( 6, TIMER_UPTIME, RD, uint64_t us); 								//AM系统启动时间
...
AM_DEVREG(14, AUDIO_CONFIG, RD, bool present; int bufsize);    //声卡控制器信息
AM_DEVREG(15, AUDIO_CTRL,   WR, int freq, channels, samples);  //声卡控制寄存器
AM_DEVREG(16, AUDIO_STATUS, RD, int count);                    //声卡状态寄存器,当前流缓冲区已经使用的大小count.
AM_DEVREG(17, AUDIO_PLAY,   WR, Area buf);                     //声卡播放寄存器

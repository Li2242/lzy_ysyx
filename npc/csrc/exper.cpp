...
void init_regex() {
  ...
  for (i = 0; i < NR_REGEX; i ++) {
    ...
    if (ret != 0) {
      ...
      green_printf("regex compilation failed: %s\n%s\n", error_msg, rules[i].regex);
			assert(0);
    }
  }
}
      ...
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        switch (rules[i].token_type) {
          ...
          default:
            ...
            green_printf("The token in the expression are not matched!\n");
            return false;
            break;
        }
        //对应上了直接退出，不需要继续匹配了；
  ...
  if (p > q) {
    ...
    */
    ...
    int op = find_main_op(p,q);
     if(op==-1){
        green_printf("Error: No operator found between %d and %d\n", p, q);
         return false;
     }
     ...
     if(tokens[op].type == TK_PT){
      ...
      switch (tokens[op].type) {
        ...
        case '/':
          if(val2 == 0){
            green_printf("Error: Division by zero\n");
            *success = false;
            return 0;
          }
          return val1/val2;

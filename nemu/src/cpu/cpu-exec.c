...
//环形缓冲区
#define MAX_INST_LEN 150
int ring_buf_count = 0;
char ring_buf[8][MAX_INST_LEN];
    ...
    if (nemu_state.state != NEMU_RUNNING){
        ...
        if(good == 0){
            ...
            for(int j = 0; j < 8; j++){
				...
				ring_buf_count = (ring_buf_count + 1) % 8;
            }
        }
        break;
    }
...
//ringbuf
void ring_buf_fun(char* inst){
	strncpy(ring_buf[ring_buf_count], inst, MAX_INST_LEN );
    ring_buf[ring_buf_count][MAX_INST_LEN - 1] = '\0';
	ring_buf_count = (ring_buf_count + 1) % 8;
}

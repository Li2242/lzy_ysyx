...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (int32_t)src1 / (int32_t)src2); //补码除法就是直接除
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->dnpc ; s->dnpc = (src1 + imm)&~1);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 < imm) );
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm);
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1+imm,2),16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = (SEXT(Mr(src1+imm,2),16))&0xFFFF);
  ...
  //J型
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->dnpc; s->dnpc = imm + s->pc);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((int32_t)src1 < (int32_t)src2) s->dnpc = imm + s->pc); 
   ...
   //虽 I型 特殊
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = src1 >> (imm & 0x1F));
  ...
  //crc32
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (int32_t)((uint32_t)src1 >> (imm & 0x1F))); //这个写的有点抽象啊

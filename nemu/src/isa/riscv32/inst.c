...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = SEXT(Mr(src1 + imm, 1),8));
  ...
  // INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = BITS(src1 << src2,4,0));
  // INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(rd) = src1 & src2);
  // INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , R, R(rd) = src1 ^ imm);

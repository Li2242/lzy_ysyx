...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (int32_t)src1 >> (src2&0x1F)); //逻辑右移
 	...
 	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu   , R, R(rd) = (int32_t)( ( (uint64_t)(uint32_t)src1*(uint64_t)(uint32_t)src2 ) >>32 ));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> (imm & 0x1F)); //算数右移
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (int32_t)((uint32_t)src1 >> (imm & 0x1F)));

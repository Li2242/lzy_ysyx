...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = src1 / src2); //补码除法就是直接除
  ...
  // INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if(src1 < src2) s->dnpc = imm + s->pc); 
  //crc32
 INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = (imm&0xFFFFF000));
